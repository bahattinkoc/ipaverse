...
- Authentication uses Apple's GrandSlam SRP-6a flow. The raw password is used locally to produce the SRP proof and is not sent directly. By default, ipaverse tries macOS's AOSKit first and automatically uses an Anisette V3 service if local generation fails, including on affected macOS 27 betas. The login screen explains this service use. Remote generation sends a generated device identity and provisioning data, never Apple Account credentials or verification codes. The default service is operated by SideStore. **Settings → Account → Advanced Sign-in Settings** lets you restrict generation to this Mac or configure your own server; existing choices are preserved. See [Anisette sign-in options](docs/anisette.md).
