...
- Select two available IPA copies with the same bundle ID using Command-click and choose **Compare**, or use **Compare With…** from a copy's context menu. The comparison opens in a separate window with metadata, entitlements, signing/provisioning, Mach-O, components, resources, file hashes/sizes and static-analysis findings. **Run Deep Analysis** adds symbols, classes/selectors, optional r2 methods, strings and endpoint candidates. Inspect A/B values or line diffs, filter by source, and export full or filtered JSON. Configuration and entitlement values are included; security finding evidence remains redacted/fingerprinted. See [IPA comparison](docs/ipa-comparison.md) for scope and limits.
...
- In Compare, selecting **Assets.car** shows changed asset names and variant-specific A/B image cards. Bitmap, SVG/PDF and color renditions are previewed when the system decoder supports them; unavailable renditions show their decode status. These catalog changes and previews are also included in JSON exports.
- Image and asset cards offer **Overlay**, **Swipe** and **Difference** in addition to A/B. Pixel differences are highlighted on the previews (up to 512 px), including alpha changes.
- **Size impact** ranks components and files by extracted byte delta, with A/B totals and separate growth/reduction. Select a component to see its files, then a file to inspect its content changes. Hover values for exact bytes; use Growth/Reduction and path filters to narrow the list.
