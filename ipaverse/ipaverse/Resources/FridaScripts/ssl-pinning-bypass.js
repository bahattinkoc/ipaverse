...
if (evalWithError) {
  Interceptor.replace(evalWithError, new NativeCallback(function (trust, error) {
...
if (evalLegacy) {
  Interceptor.replace(evalLegacy, new NativeCallback(function (trust, result) {
    send('[ssl-pinning-bypass] SecTrustEvaluate -> kSecTrustResultProceed');
    if (result && !result.isNull()) { result.writeU32(1); }
    return 0;
  }, 'int', ['pointer', 'pointer']));
  send('[ssl-pinning-bypass] hooked SecTrustEvaluate');
}
if (!evalWithError && !evalLegacy) { send('[ssl-pinning-bypass] no SecTrust export found'); }
